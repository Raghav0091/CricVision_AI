...
function TrackingResult({
  ...
  const stats = [
    ...
    ["Predicted points", summary.predicted_points.toLocaleString()],
    ["Rejected candidates", summary.rejected_candidates.toLocaleString()],
  ...
  const links = [
    ...
    ["Open tracking video", summary.tracking_video_url]
  ];
    ...
    <div className="mt-6 space-y-6">
      ...
      <div className="grid gap-5 xl:grid-cols-3">
        ...
        <div className="rounded-xl border border-lime/20 bg-lime/[0.03] p-4">
          <p className="text-xs font-bold uppercase tracking-[0.15em] text-lime">Moving Ball Tracking Video</p>
      ...
      <div className="flex flex-wrap gap-4 rounded-xl border border-white/10 bg-black/20 p-4 text-xs text-white/55">
        ...
        <span><i className="mr-2 inline-block h-3 w-3 rounded-full border-2 border-[#ffe600]" />Yellow = predicted</span>
        <span><i className="mr-2 inline-block h-3 w-3 rounded-full border-2 border-[#ff9600]" />Orange = recovered estimate</span>
        <strong className="text-white/70">This is tracking diagnostics, not a final trajectory.</strong>
      </div>
      ...
      <div className="grid gap-4 rounded-xl border border-white/10 bg-black/20 p-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <div><span className="block text-xs text-white/35">Possible bounce transition</span><strong className="mt-1 block text-sm capitalize">{String(summary.possible_bounce_transition_detected)}</strong></div>
    ...
    <Card className="border-lime/20">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          />
          <h2 className="mt-4 text-2xl font-black">Moving Ball Tracker v1</h2>
        ...
        <Button disabled={active} onClick={() => void runTracking()}>
          {starting ? "Starting…" : active ? "Tracker running…" : result ? "Run Moving Ball Tracker Again" : "Run Moving Ball Tracker"}
        </Button>
      </div>
