...
def initialise_video_calibration_v2(
    ...
    landmarks = [
        *_landmarks_from_wicket_box(
            ...
            image_height,
            world_points,
            bowler_wicket.confidence if bowler_wicket is not None else None,
        ),
        *_landmarks_from_wicket_box(
            "striker",
            striker_box.x,
            ...
            image_height,
            world_points,
            striker_wicket.confidence if striker_wicket is not None else None,
        ),
    ]
...
def confirm_video_calibration_v2(
    ...
    status = (
        "confirmed"
        if homography.transform_available
        else "insufficient_geometry"
    )
    message = (
        "Calibration v2 ground-plane geometry confirmed."
        if status == "confirmed"
        else (
            "Calibration landmarks were saved, but the ground-plane geometry "
            "is insufficient for a stable homography."
        )
    )
...
def _calculate_ground_homography(
    ...
    try:
        image_to_ground, _ = cv2.findHomography(source, destination, method=0)
    except cv2.error:
        image_to_ground = None
    if image_to_ground is None or image_to_ground.shape != (3, 3):
    ...
    image_to_ground = image_to_ground.astype(np.float64)
    if abs(image_to_ground[2, 2]) > 1e-12:
    ...
    diagnostics = _reprojection_diagnostics(landmarks, ground_to_image)
    ...
    warnings.append(
        "Homography maps only the pitch ground plane (Z = 0), not airborne ball height."
    )
...
def _project_virtual_pitch_geometry(
    ...
    if ground_to_image_homography is None:
        return VirtualPitchOverlayGeometry(projected_lines=[])
    labels = {
    ...
    return VirtualPitchOverlayGeometry(projected_lines=lines)
...
def _overall_confidence(
    ...
    coverage = min(1.0, image_coverage / 0.05)
    condition = {
    ...
    try:
        ...
        for line in result.virtual_pitch_overlay_geometry.projected_lines:
            ...
            if len(points) >= 2:
                draw.line(
                    points,
                    fill=line_colors.get(line.id, (213, 255, 107, 200)),
                    width=line_width,
                )
        ...
        draw.rectangle((10, 10, 285, 55), fill=(8, 12, 16, 220))
        draw.text(
            (18, 17),
            "Calibration v2 - Ground Plane",
            fill=(255, 255, 255, 255),
        )
        draw.text(
            (18, 35),
            f"Quality: {result.quality.quality_grade}",
            fill=(213, 255, 107, 255),
        )
...
def _landmarks_from_wicket_box(
    ...
    confidence = (
        max(0.15, min(0.65, detected_confidence * 0.6))
